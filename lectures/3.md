...
Suppose, we have a pairwise distance matrix for $N$ $d$-dimensional objects $D \in \mathbb{R}^{N \times N}$. Given this matrix, we aim to recover the initial coordinates $W_i \in \mathbb{R}^d, \; i = 1, \ldots, N$.
...
![[Link to the animation](https://fmin.xyz/docs/visualizations/mds.mp4)](mds.png){width=40%}
...
## The curse of dimensionality for zero-order methods ^[[Optimal rates for zero-order convex optimization: the power of two function evaluations](https://arxiv.org/pdf/1312.2139)]
...
## Neural network training dynamics via Hessian spectra and hvp ^[[An Investigation into Neural Net Optimization via Hessian Eigenvalue Density](https://arxiv.org/abs/1901.10159)]

![Large negative eigenvalues disappeared after training for ResNet-32](ResNet_32_before_After.png)
...
Real world example from **GPT-2**^[[ZeRO: Memory Optimizations Toward Training Trillion Parameter Models](https://arxiv.org/abs/1910.02054)]: 
...
* I recommend reading the official Jax Autodiff Cookbook. [Open In Colab $\clubsuit$](https://colab.research.google.com/github/MerkulovDaniil/optim/blob/master/assets/Notebooks/Autograd_and_Jax.ipynb)
